...
export default function InvitePage() {
  return (
    <Suspense
      fallback={
        <div
          className="min-h-dvh flex items-center justify-center font-[family-name:var(--font-inter)]"
          style={{
            background:
              "linear-gradient(135deg, #0F1B3D 0%, #1A3A6E 30%, #2E6BB5 60%, #2E6BB5 100%)",
          }}
        >
          <div className="animate-pulse w-80 h-64 rounded-3xl bg-white/10" />
        </div>
      }
    >
      <InvitePageInner />
...
function InvitePageInner() {
  ...
  const { session, loading: authLoading, profileData } = useAuth();
  ...
  function openAuthModal(mode: "signin" | "signup") {
    // Store invite code so we can auto-accept after auth
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 85% 130%, #F4B084 0%, #E8956D 25%, rgba(46,107,181,0) 60%)",
        }}
      />
      {/* Sheen */}
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 30% -20%, rgba(255,255,255,0.15) 0%, transparent 50%), radial-gradient(ellipse at 70% 110%, rgba(255,255,255,0.06) 0%, transparent 40%)",
        }}
      />

      {/* Logo */}
      <div className="relative z-10 mb-8">
        <a href="/" className="text-white text-2xl font-extrabold tracking-tight">
          Elena
        </a>
      </div>
          ...
          {!isLoading && fetchError && (
            <div className="text-center space-y-4">
              <div className="w-14 h-14 rounded-full bg-red-500/20 flex items-center justify-center mx-auto">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-red-300">
              ...
              <a
                href="/"
                className="inline-block mt-4 rounded-full bg-white/95 px-6 py-3 text-sm font-semibold text-[#0F1B3D] hover:bg-white transition-colors"
              >
                Go to Elena
              </a>
            </div>
            ...
            <div className="text-center space-y-5">
              ...
              <div>
                ...
                <p className="text-white/40 text-sm mt-3">
                  Connect on Elena to share health information and support each other.
                </p>
              </div>
              ...
              <div className="space-y-3 pt-2">
                <button
                  ...
                  className="w-full rounded-full bg-white/95 py-3.5 text-sm font-semibold text-[#0F1B3D] hover:bg-white transition-colors shadow-[0_4px_16px_rgba(0,0,0,0.1)]"
                >
                  Sign up to connect
                </button>
                <button
                  onClick={() => openAuthModal("signin")}
                  className="w-full rounded-full border border-white/20 bg-white/5 py-3.5 text-sm font-semibold text-white hover:bg-white/10 transition-colors"
                >
            ...
            <div className="text-center space-y-5">
              ...
              <div className="space-y-3 pt-2">
                <button
                  ...
                  disabled={accepting}
                  className="w-full rounded-full bg-white/95 py-3.5 text-sm font-semibold text-[#0F1B3D] hover:bg-white transition-colors shadow-[0_4px_16px_rgba(0,0,0,0.1)] disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={declining}
                  className="w-full rounded-full border border-white/20 bg-white/5 py-3.5 text-sm font-semibold text-white/60 hover:bg-white/10 hover:text-white transition-colors disabled:opacity-50"
                >
            ...
            <div className="text-center space-y-4">
              <div
                className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto ${
                  actionResult.type === "success" ? "bg-green-500/20" : "bg-red-500/20"
                }`}
              >
                {actionResult.type === "success" ? (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-green-300">
              ...
              <h2 className="text-xl font-bold text-white">
                {actionResult.type === "success" ? "Connected!" : "Something went wrong"}
              </h2>
              <p className="text-white/50 text-sm">{actionResult.message}</p>
              {actionResult.type === "error" && (
                <button
                  onClick={() => setActionResult(null)}
                  className="inline-block mt-2 rounded-full bg-white/95 px-6 py-3 text-sm font-semibold text-[#0F1B3D] hover:bg-white transition-colors"
                >
                  Try again
                </button>
              )}
      ...
      <AuthModal
        ...
        onOpenChange={(open) => {
          ...
          // If the modal closed and user is now logged in, the auto-accept effect will handle it
        }}
