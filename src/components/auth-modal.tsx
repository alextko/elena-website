...
export function AuthModal({
  ...
  oauthRedirectTo,
  defaultMode = "signup",
  /** Raises both overlay and content z-index above the default Radix
   *  stack (z-50). Use when mounting AuthModal on a surface that
   *  already has high-z content (e.g. the tour renders at z-[99999]).
   *  Without this, the modal renders behind the host surface and the
   *  user sees a darkened backdrop but no dialog. */
  elevateAboveHighZIndex = false,
}: {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  oauthRedirectTo?: string;
  defaultMode?: "signin" | "signup";
  elevateAboveHighZIndex?: boolean;
}) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      {open && (
        <style>{`
          ...
          /* elevateAboveHighZIndex z-index handling moved to inline
             style + overlayClassName on DialogContent — inline style
             beats any base class, and Tailwind's ! prefix on
             overlayClassName beats DialogOverlay's default z-50. */
        `}</style>
      )}
      <DialogContent
        overlayClassName={elevateAboveHighZIndex ? "!z-[2147483000]" : undefined}
        style={elevateAboveHighZIndex ? { zIndex: 2147483001 } : undefined}
        className={`max-sm:w-[75vw] w-[calc(100%-2rem)] max-w-lg max-h-[90dvh] overflow-y-auto p-0 border-0 rounded-3xl shadow-[0_16px_48px_rgba(0,0,0,0.25)] font-[family-name:var(--font-inter)] [&_[data-slot=dialog-close]]:text-white/70 [&_[data-slot=dialog-close]:hover]:text-white [&_[data-slot=dialog-close]:hover]:bg-white/10 max-md:!fixed max-md:!inset-0 max-md:!top-0 max-md:!left-0 max-md:!translate-x-0 max-md:!translate-y-0 max-md:!w-full max-md:!max-w-none max-md:!max-h-none max-md:!h-full max-md:!rounded-none max-md:!shadow-none max-md:!border-0 ${elevateAboveHighZIndex ? "" : "max-md:!z-[200]"}`}>
