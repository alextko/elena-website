...
/**
 * Tests that EVERY suggestion button across ALL demo scenarios hands off
 * to the real agent (no self-matches, no errors, no generic fallback).
 */
...
// ---------------------------------------------------------------------------
// Tests: one per scenario, clicks FIRST suggestion and verifies handoff
// ---------------------------------------------------------------------------

test.describe("Demo suggestions: first suggestion hands off correctly", () => {
...
// ---------------------------------------------------------------------------
// Tests: verify ALL suggestions for each scenario don't self-match
// ---------------------------------------------------------------------------

test.describe("Demo suggestions: no suggestion self-matches a demo entry", () => {
  for (const scenario of SCENARIOS) {
    for (const suggestion of scenario.suggestions) {
      test(`${scenario.demoName}: "${suggestion}" does not self-match`, async ({ page }) => {
        const mock = createMock();
        await injectAuthAndDemoMode(page);
        await setPendingQueryAndDoc(page, scenario.initialQuery, scenario.initialDoc);
        await mock.setup(page);
